 ...
 *
 */
public class TriangReduction implements Inference {
	...
	public String getName() {
		return "Approximate Inference by Edge Deletion for Bounded Treewidth";
	}
	...
	public void applyReductionRules() {
		...
		while (moralizedGraph.getNumberOfVertices() != 0) {
			...
			//System.out.println("remaining edges in reduced graph: " + moralizedGraph.getNumberOfEdges());
			//System.out.println("edges in orig graph: " + origNetwork.getGraph().getNumberOfEdges());
			removeEdge();
			appliedRule = false;
			//appliedRule = isletRule();
			if (!appliedRule) {
			...
			/*if (oldEdges == moralizedGraph.getNumberOfEdges() && !appliedRule) {
				break;
			}*/
			count++;
		}
		
		//System.out.println("deleted: " + removedVertices.size());
		//System.out.println("edges deleted: " + count);
		long reductStop = System.currentTimeMillis();
		//System.out.println("reduction rules time: " + (reductStop-reductStart));
		//printGraph();
		//System.out.println("\nMORALS:");
		//printMorals();
		//System.out.println("\nORIG GRAPH:");
	...
	private void removeEdge() {
		...
		for (int i = 0; i < moralizedGraph.getVertices().length; i++) {
			...
			for (int j = 0; j < moralizedGraph.getChildren(v1).length; j++) {
				...
				if (isOrigEdge(getOrigVertex(v1.getName()), getOrigVertex(v2.getName())) &&
						!isOrigAndMoral(v1.getName(), v2.getName())) {
		...
		if (bestSource != null && bestSink != null) {
			//System.out.println("\tremoving edge: " + bestSource.getName()+"<-->"+bestSink.getName());
			deleteExtraMorals();
			...
			//System.out.println("\tremoving orig edge: " + bestSource.getName()+"<-->"+bestSink.getName());
			deleteExtraMorals();
			deleteFromOrig(bestSource.getName(), bestSink.getName());
			deleteFromMoral(bestSource.getName(), bestSink.getName());
		}
			...
			if (moralEdges.contains(source.getName()+"->"+delSource.getName()) && 
				...
				if (moralSink.equals(sink.getName())) {
					...
					if (moral1 != null && moral2 != null) {
						moralizedGraph.removeEdge(getMoralVertex(source.getName()), 
								getMoralVertex(delSource.getName()));
						moralizedGraph.removeEdge(getMoralVertex(delSource.getName()), 
								getMoralVertex(source.getName()));
					}
	...
	private boolean simplicialRule() {
		...
		for (int i = 0; i < moralizedGraph.getVertices().length; i++) {
			...
			if (isSimplicial(current) && moralizedGraph.getParents(current).length <= low &&
					moralizedGraph.getParents(current).length <= 4) {
	...
	private void connect(Vertex bn1, Vertex bn2) {
		//String name1 = bn1.getName();
		//String name2 = bn2.getName();
		moralizedGraph.addUndirectedEdge(bn1, bn2);
		return;
		
//		for (int i = 0; i < moralizedGraph.getNodes().length; i++) {
//			for (int j = 0; j < moralizedGraph.getNodes().length; j++) {
//				if (moralizedGraph.getNodes()[i].getName().equals(name1) &&
//						moralizedGraph.getNodes()[j].getName().equals(name2)) {
//					moralizedGraph.connect(moralizedGraph.getNodes()[i],
//										   moralizedGraph.getNodes()[j]);	
//					moralizedGraph.connect(moralizedGraph.getNodes()[j],
//							   			   moralizedGraph.getNodes()[i]);
//					System.out.println("\t\tdone in connect");
//					return;
//				}
//				System.out.println("\t\tconnect iteration " + i + " " + j);
//			}
//		}
	}
	...
	private boolean buddyRule() {
		...
		for (int i = 0; i < moralizedGraph.getVertices().length; i++) {
			for (int j = i+1; j < moralizedGraph.getVertices().length; j++) {
				...
				if (moralizedGraph.getParents(node1).length == 3 && 
						moralizedGraph.getParents(node2).length == 3 &&
						sameParents(node1, node2)) {
	...
	private boolean cubeRule() {
		...
		for (int i = 0; i < moralizedGraph.getVertices().length; i++) {
			...
			if (moralizedGraph.getParents(d).length == 3) {
				...
				if (moralizedGraph.getParents(a).length == 2 &&
						moralizedGraph.getParents(b).length == 2 &&
						moralizedGraph.getParents(c).length == 2) {
					...
					if (moralizedGraph.getParents(v).length != 2 ||
							moralizedGraph.getParents(w).length != 2 ||
							moralizedGraph.getParents(x).length != 2) {
						break;
					}
					if (!v.getName().equals(w.getName()) && !v.getName().equals(x.getName()) && 
							!w.getName().equals(x.getName())) {
						connect(w, v);
						connect(v, x);
						connect(w, x);
	...
	public void doInference() {
		...
		for (int i = 0; i < moralizedGraph.getVertices().length; i++) {
			...
			/*for (int j = 0; j < moralizedGraph.getChildren(moralizedGraph.getVertices()[i]).length; j++) {
				deleteFromOrig(moralizedGraph.getVertices()[i].getName(), 
							   moralizedGraph.getChildren(moralizedGraph.getVertices()[i])[j].getName());
				count++;
			}*/
		}
		//System.out.println("edges deleted: " + count/2);
	...
	private void disconnectFromOrig(String name) {
		...
		for (int i = 0; i < parents.length; i++) {
			if (!inReducedGraph(parents[i].getName())) {
				...
				network.removeEdge(network.getVertices()[parents[i].loc()],
						network.getVertices()[node1.loc()]);
				network.removeEdge(network.getVertices()[node1.loc()],
						network.getVertices()[parents[i].loc()]);
				removeSource = getOrigVertex(node1.getName());
				removeSink = getOrigVertex(parents[i].getName());
				//System.out.println("disconnecting: " + removeSource.getName() + "<-->" + removeSink.getName());
				deleteExtraMorals();
			}
		}
		for (int i = 0; i < children.length; i++) {
			if (!inReducedGraph(children[i].getName())) {
				...
				network.removeEdge(network.getVertices()[children[i].loc()],
						network.getVertices()[node1.loc()]);
				network.removeEdge(network.getVertices()[node1.loc()],
						network.getVertices()[children[i].loc()]);
			}
		}
	}
	...
	private void calcAlpha() {
		...
		/*for (int i = 0; i < alpha.length; i++) {
			System.out.println(alphainv[i+1].getName());
		}*/
	}
	...
	//this version of deleteNode ignores the CPF adjustments
	...
	public static void main(String[] args) {
		...
		TriangReduction tr = new TriangReduction();
		tr.run(bn);
